ï»¿//===--- RuleHeadlineTexts.cpp - clang-tidy-misra -------------------------===//
//
...
namespace emboldCxx {
	extern const HeadlineByRuleName ruleHeadlines = {
		...
		// Postfix expressions
		{ "misra-cpp2008-5-2-1", R"ruletext(Each operand of a logical && or || shall be a postfixâexpression.)ruletext" },
		...
		{ "misra-cpp2008-6-2-3", R"ruletext(Before preprocessing, a null statement shall only occur on a line by itself; it may be followed by a comment, provided that the first character following the null statement is a whiteâspace character.)ruletext" },
		...
		{ "misra-cpp2008-6-4-5", R"ruletext(An unconditional throw or break statement shall terminate every nonâempty switch-clause.)ruletext" },
		...
		//Custom checks
		{ "global-variable", R"ruletext(Avoid using global variables)ruletext" },
		{ "bugprone-assignment-in-if-condition", R"ruletext("an assignment within an 'if' condition is bug-prone")ruletext" },
		{ "empty-catch", R"ruletext(catch block should not be empty.)ruletext" },
		{ "deactivated-code", R"ruletext(Mechanisms preventing code from being compiled and executed.)ruletext" },
		{ "fixed-size-memory-operation", R"ruletext(Memory operation is called with a hard-coded length instead of using a macro or constant or sizeof().)ruletext" },
		...
		{ "misra-c2012-21.8", R"ruletext(The library functions abort, exit and system of <stdlib.h> shall not be used)ruletext" },
